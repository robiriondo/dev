...
window.App = App = (function() {
    ...
    App = (function() {
        ...
        function App() {
            ...
            var clicker = function clicker(name, locations) {
                return function() {
                    ...
                    return saveThis.setLocations(locations);
                };
            };
            ...
            */
            ...
            this.keyPressLock = 0;
            //TODO: Debug Navigation function
            $(document).keydown(function(e) {
              if (saveThis.keyPressLock === 0) {
                //var _ = void 0, wk = void 0;
                // Arrow left
                if (e.keyCode === 37) {
                  console.log("Left arrow pressed.");
                  saveThis.keyPressLock = 1;

                  var _Array$from23 = Array.from(epiweekOffByOne(saveThis.currentEpweek));

                  saveThis.currentEpweek = _Array$from23[0];
                  _ = _Array$from23[1];
                  wk = saveThis.currentEpweek % 100;

                  if (wk === 39) {
                    saveThis.nonInfluenzaWeekSeason = saveThis.nonInfluenzaWeekSeason - 1;
                  }

                  saveThis.loadEpidata(saveThis.currentEpweek);

                  if (saveThis.currentDetailedLoc != null) {
                    saveThis.fetchNowcast(
                      saveThis.currentDetailedLoc,
                      saveThis.currentEpweek
                    );
                  } else {
                    // TODO: Debugging
                    console.log("Debugging: No data found.");
                  }
                }
                // Arrow up
                if (e.keyCode === 38) {
                  ...
                  if (saveThis.currentEpweek < saveThis.maxEpiweek) {
                    saveThis.keyPressLock = 1;

                    var _Array$from24 = Array.from(epiweekOffByTen(saveThis.currentEpweek));

                    _ = _Array$from24[0];
                    saveThis.currentEpweek = _Array$from24[1];
                    saveThis.currentEpweek = Math.min(
                      saveThis.currentEpweek,
                      saveThis.maxEpiweek
                    );
                    wk = saveThis.currentEpweek % 100;
                    ...
                    wk = saveThis.maxEpiweek % 100;
                    //alert("Week" + wk + " is the lastest data we had! Please check back next week!");
                    console.log(
                      "Week" + wk + " is the lastest data we had! Please check back next week!"
                    );
                  }
                }
                // Arrow right
                if (e.keyCode === 39) {
                  ...
                  if (saveThis.currentEpweek < saveThis.maxEpiweek) {
                    ...
                    var _Array$from25 = Array.from(epiweekOffByOne(saveThis.currentEpweek));

                    _ = _Array$from25[0];
                    saveThis.currentEpweek = _Array$from25[1];
                    wk = saveThis.currentEpweek % 100;
                ...
                if (e.keyCode === 40) {
                  ...
                  var _Array$from26 = Array.from(epiweekOffByTen(saveThis.currentEpweek));

                  saveThis.currentEpweek = _Array$from26[0];
                  _ = _Array$from26[1];
                  wk = saveThis.currentEpweek % 100;
        ...
        App.prototype.loadEpidata = function loadEpidata() {
            ...
            if (epweek != null) {
                ...
                callback1 = function callback1(epidata) {
                    var NonInfluenzaData = calculateNonInfluenzaData(epidata, saveThis.nonInfluenzaWeekSeason);
                    var callback2 = function callback2(epidata) {
                        ...
                        for (var _iterator19 = Array.from(epidata), _isArray19 = Array.isArray(_iterator19), _i20 = 0, _iterator19 = _isArray19 ? _iterator19 : _iterator19[Symbol.iterator](); ; ) {
                            ...
                            if (row.epiweek === epiweek2) {
                                ...
                                saveThis.colors[row.location] = "#" + c + "4040";
                                saveThis.mapData[row.location] = row;
                            }
                        }
                        // global_epiweek = epiweek2
                        saveThis.dataTimeline.html("Nowcasting epi-week " + (epiweek2 % 100) + " " + datestr);
                        if (saveThis.keyPressLock === 1) {
                            return (saveThis.keyPressLock = 0);
                        }
                    };
                ...
                var callback = function callback(epidata) {
                    ...
                    saveThis.maxEpiweek = epiweek2;
                    date = epiweek2date(epiweek2);
                    datestr = "(" + date2String(date);
                    date.setDate(date.getDate() + 6);
                    datestr = datestr + "-" + date2String(date) + ")";
                    global_epiweek = epiweek2 //Removing this line breaks the rendering of ILI Value on Map
                    //Display nowcasting week on map app
                    saveThis.dataTimeline.html("Nowcasting epi-week " + (epiweek2 % 100) + " " + datestr);
                    saveThis.currentEpweek = epiweek2;
                    callback1 = function callback1(epidata) {
                        var NonInfluenzaData = calculateNonInfluenzaData(epidata, saveThis.nonInfluenzaWeekSeason);
                        var callback2 = function callback2(epidata) {
                            saveThis.colors = {};
                    ...
                    ;
                    handler = getEpidataHander(callback1);
                    return Epidata_nowcast_multi(handler, STATES, saveThis.nonInfluenzaWeekSeason * 100 + 40, (saveThis.nonInfluenzaWeekSeason + 1) * 100 + 39);
                };
        ...
        */
        ...
        //Add compatibility with Leaflet, show location details
        ...
        App.prototype.onNowcastReceived = function onNowcastReceived(epidata, currentEpiweek) {
            ...
            if (Array.from(REGIONS).includes(loc) || Array.from(NATIONAL).includes(loc)) {
                var callback = function callback(ilidata) {
                    return saveThis.onFluviewReceived(ilidata);
                };
	...
	for (var i=0; i<epidata.length; i++) {
        ...
        chart = d3_timeseries().
          addSerie(epidata, {
              ...
              interpolate: "monotone", //linear default
              color: "#c00",
          })
